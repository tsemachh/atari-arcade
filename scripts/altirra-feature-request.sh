...
joystick, so an embedder cannot feed stick input from JS at all.
...
input sources the core does not ship: device-orientation (tilt) steering,
...
There is no exported save/load-state function, so I cannot work around this
...
opacity slider dims the joystick/fire too, so it is not a substitute.
...
if [ -n "$ISSUE" ]; then
  ...
  gh issue edit "$ISSUE" --repo "$REPO" --title "$TITLE" --body-file "$BODY_FILE"
  gh issue view "$ISSUE" --repo "$REPO" --web
else
  echo "Creating a new issue on $REPO ..."
  gh issue create --repo "$REPO" --title "$TITLE" --body-file "$BODY_FILE" --web
fi
